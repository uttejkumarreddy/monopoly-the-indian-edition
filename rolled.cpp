    ...
    ui(new Ui::rolled)
{
    ...
    ui->dice1->setNum(die1);
    ui->dice2->setNum(die2);
    ui->landedspace->setText(opdata.card_name(gamers[opdata.currently_playing()].get_position()));
    ui->place_price->setNum(cardsinfo[gamers[opdata.currently_playing()].get_position()].get_price());
    ui->roll_wallet->setNum(gamers[opdata.currently_playing()].get_money());
...
void rolled::on_avail_skip_clicked()
{
    hide();
...
void rolled::on_avail_buy_clicked()
{
    int position, money_to_be_subtracted;
    position = gamers[opdata.currently_playing()].get_position();
    money_to_be_subtracted = cardsinfo[position].get_price();
    if(gamers[opdata.currently_playing()].get_money() < money_to_be_subtracted)
    {
        opdata.increment_current_player();
    ...
    {
    gamers[opdata.currently_playing()].subtract_money(money_to_be_subtracted);
    gamers[opdata.currently_playing()].add_possessions(position);
    cardsinfo[position].update_possession(opdata.currently_playing());
    gamers[opdata.currently_playing()].increment_possessionnumber();
